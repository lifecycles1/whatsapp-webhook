export function datastore() {
  ...
  // fill empties not in use anymore
  const fillEmpties = (array) => {
    array.forEach((arr) => {
      if (arr.range.startsWith(`${sheets.q1}`) || arr.range.startsWith(`${sheets.q2}`)) {
        arr.values.forEach((val) => {
          if (val.length === 0) {
            val[0] = null;
          }
        });
      } else if (arr.range.startsWith(`${sheets.q4}`) || arr.range.startsWith(`${sheets.q5}`)) {
        arr.values.forEach((val) => {
          if (val.length === 0) {
            val[0] = null;
          }
